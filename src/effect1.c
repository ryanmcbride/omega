...
/* bless */
void bless(blessing) int blessing;
{
  int index, used;
...
void fbolt(fx, fy, tx, ty, hit, dmg) int fx, fy, tx, ty, hit, dmg;
{
  bolt(fx, fy, tx, ty, hit, dmg, FLAME);
}

void lbolt(fx, fy, tx, ty, hit, dmg) int fx, fy, tx, ty, hit, dmg;
{
  bolt(fx, fy, tx, ty, hit, dmg, ELECTRICITY);
}

/* Added 12/30/98 DG */
void icebolt(fx, fy, tx, ty, hit, dmg) int fx, fy, tx, ty, hit, dmg;
{
  bolt(fx, fy, tx, ty, hit, dmg, COLD);
}

void nbolt(fx, fy, tx, ty, hit, dmg) int fx, fy, tx, ty, hit, dmg;
{
  bolt(fx, fy, tx, ty, hit, dmg, NORMAL_DAMAGE);
}

/* from f to t */
void bolt(fx, fy, tx, ty, hit, dmg, dtype) int fx, fy, tx, ty, hit, dmg, dtype;
{
  int xx, yy;
...
void lball(fx, fy, tx, ty, dmg) int fx, fy, tx, ty, dmg;
{
  ball(fx, fy, tx, ty, dmg, ELECTRICITY);
}

void manastorm(x, y, dmg) int x, y, dmg;
{
  ball(x, y, x, y, dmg, UNSTOPPABLE);
}

void snowball(fx, fy, tx, ty, dmg) int fx, fy, tx, ty, dmg;
{
  ball(fx, fy, tx, ty, dmg, COLD);
}

void fball(fx, fy, tx, ty, dmg) int fx, fy, tx, ty, dmg;
{
  ball(fx, fy, tx, ty, dmg, FLAME);
}

/* from f to t */
void ball(fx, fy, tx, ty, dmg, dtype) int fx, fy, tx, ty, dmg, dtype;
{
  int xx, yy, ex, ey, i;
  struct monster *target;
...
void mondet(blessing) int blessing;
{
  pml ml;
...
void objdet(blessing) int blessing;
{
  int i, j;
  for (i = 0; i < WIDTH; i++)
    for (j = 0; j < LENGTH; j++)
...
void identify(blessing) int blessing;
{
  int index;
...
void wish(blessing) int blessing;
{
  int i;
  char wishstr[80];
  clearmsg();
...
/* gain for an item */
void acquire(blessing) int blessing;
{
  char otype;
  int index, id = ABORT;
  pob newthing;
