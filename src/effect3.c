...
void summon(blessing, id) int blessing, id;
{
...
int itemlist(itemindex, num) int itemindex, num;
{
  int i, itemno;
...
void cleanse(blessing) int blessing;
{
  int i;
...
void annihilate(blessing) int blessing;
{
  pml ml;
  int i;
...
void sleep_monster(blessing) int blessing;
{
  pml ml;
...
void sleep_player(amount) int amount;
{
...
void clairvoyance(vision) int vision;
{
  int i, j;
  int x = Player.x, y = Player.y;
  mprint("Clairvoyance... ");
...
void learnspell(blessing) int blessing;
{
...
/*affects player only */
void level_drain(levels, source) int levels;
char *source;
{
...
void disrupt(x, y, amount) int x, y, amount;
{
  struct monster *target;
...
void p_poison(toxicity) int toxicity;
{
...
void apport(blessing) int blessing;
{
...
void strategic_teleport(blessing) int blessing;
{
  int new_env;
...
void hero(blessing) int blessing;
{
  if (blessing > -1)
  {
...
void levitate(blessing) int blessing;
{
  if (blessing > -1)
  {
...
void cure(blessing) int blessing;
{
  int happened = FALSE;
  if (blessing > -1)
  {
...
void disease(amount) int amount;
{
  mprint("You feel ill.");
...
void truesight(blessing) int blessing;
{
  if (blessing > -1)
  {
...
void dispel(blessing) int blessing;
{
...
void polymorph(blessing) int blessing;
{
...
void hellfire(x, y, blessing) int x, y, blessing;
{
  struct monster *m;
  if ((x == Player.x) && (y == Player.y))
  {
...
void drain(blessing) int blessing;
{
  int x = Player.x, y = Player.y;
  struct monster *m;
  setspot(&x, &y);
...
void illuminate(blessing) int blessing;
{
...
void drain_life(amount) int amount;
{
  amount = abs(amount);
...
void deflection(blessing) int blessing;
{
  if (blessing > -1)
  {
