...
FILE *checkfopen(filestring, optionstring) char *filestring, *optionstring;
{
  FILE *fd;
  char response;
...
void save_hiscore_npc(npc) int npc;
{
  FILE *infile, *outfile;
  char buffer[80];
  int i;
...
void checkhigh(descrip, behavior) char *descrip;
int behavior;
{
  long points;
...
void extendlog(descrip, lifestatus) char *descrip;
int lifestatus;
{
  FILE *fd;
  int npcbehavior;
...
#ifdef MSDOS
int test_file_access(file_name, mode) char *file_name;
char mode;
{
  FILE *fd;
...
void displayfile(filestr) char *filestr;
{
...
/* display a file given a string name of file */
void displaycryptfile(filestr) char *filestr;
{
...
void copyfile(srcstr) char *srcstr;
{
  char deststr[80];
