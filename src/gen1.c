...
void clear_level(dungeon_level) struct level *dungeon_level;
{
  int i, j;
...
void change_level(fromlevel, tolevel, rewrite_level) char fromlevel, tolevel, rewrite_level;
{
...
/* tries to find the level of depth levelnum in dungeon; if can't find
   it returns NULL */
plv findlevel(dungeon, levelnum) struct level *dungeon;
char levelnum;
{
  if (dungeon == NULL)
    return (NULL);
...
void straggle_corridor(fx, fy, tx, ty, loc, rsi) int fx, fy, tx, ty;
Symbol loc;
char rsi;
{
  int dx, dy;
...
void corridor_crawl(fx, fy, sx, sy, n, loc, rsi) int *fx, *fy, sx, sy, n;
Symbol loc;
char rsi;
{
  int i;
...
void find_stairs(fromlevel, tolevel) char fromlevel;
char tolevel;
{
  int i, j, found = FALSE;
  Symbol sitechar;
...
void build_square_room(x, y, l, rsi, baux) int x, y, l;
char rsi;
int baux;
{
  int i, j;
...
void build_room(x, y, l, rsi, baux) int x, y, l;
char rsi;
int baux;
{
...
void sewer_corridor(x, y, dx, dy, locchar) int x, y, dx, dy;
Symbol locchar;
{
  int continuing = TRUE;
  makedoor(x, y);
