...
long get_money(limit) long limit;
{
  long c;
  print1("How much? ");
  c = parsenum();
...
int key_to_index(key) signed char key;
{
  int i;
...
signed char index_to_key(index) signed int index;
{
  if (index < MAXITEMS)
...
void p_drop_at(x, y, n, o) int x, y;
int n;
pob o;
{
  pol tmp;
...
void give_money(m) struct monster *m;
{
  pob cash;
...
void givemonster(m, o) struct monster *m;
struct object *o;
{
...
void dispose_lost_objects(n, obj) int n;
pob obj;
{
...
void conform_lost_objects(n, obj) int n;
pob obj;
{
  int i, conformed = FALSE, subtracted = FALSE;
  if (obj != NULL)
...
int badobject(slotchar) char slotchar;
{
...
int baditem(slotnum) int slotnum;
{
...
void gain_item(o) struct object *o;
{
...
int use_pack_item(response, slot) int response, slot;
{
  pob item;
  int i;
...
int aux_display_pack(start_item, slot) int start_item, slot;
{
...
int aux_take_from_pack(slot) int slot;
{
...
/* takes something from pack, puts to slot, 
or to 'up-in-air', one of which at least must be empty */
int aux_top_take_from_pack(slot, display) int slot, display;
{
  char response;
...
void item_inventory(topline) int topline;
{
  if (topline)
  {
    display_possessions();
...
void drop_from_slot(slot) int slot;
{
  int n, waitflag;
  if (Player.possessions[slot] != NULL)
  {
...
void put_to_pack(slot) int slot;
{
  int waitflag, num = 1;
...
pob split_item(num, item) int num;
pob item;
{
  pob newitem = NULL;
  if (item != NULL)
...
void switch_to_slot(slot) int slot;
{
...
int objequal(o, p) struct object *o, *p;
{
  if ((o == NULL) || (p == NULL))
    return (FALSE);
  else
...
int find_item(o, id, chargeval) int id, chargeval;
pob *o;
{
  int i, found = FALSE;
  *o = NULL;
...
/* returns true if item with id and charge is found in pack or in
   ...
   corpses instead of aux, which is their food value. */
int find_and_remove_item(id, chargeval) int id, chargeval;
{
  int i, found = FALSE;
  pob o = NULL;
...
/* are involved */
void show_inventory_slot(slotnum, topline) int slotnum;
int topline;
{
  if (!topline)
