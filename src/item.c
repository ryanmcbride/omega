...
pob create_object(itemlevel) int itemlevel;
{
  pob new;
  int r;
  int ok = FALSE;
  ...
  while (!ok)
  {
    ...
    ok = ((new->uniqueness < UNIQUE_MADE) &&
          ((new->level < itemlevel + random_range(3)) || (random_range(100) == 23)));
    if (!ok)
    {
      free((char *)new);
    }
...
/* taken from Knuth 2 */
void shuffle(ids, number) int ids[];
int number;
{
  int top, swap, with;
