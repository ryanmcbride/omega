...
void m_smart_move(m) struct monster *m;
{
  m_simple_move(m);
}

/* not very smart, but not altogether stupid movement */
void m_normal_move(m) struct monster *m;
{
  m_simple_move(m);
}

/* used by both m_normal_move and m_smart_move */
void m_simple_move(m) struct monster *m;
{
...
void m_move_animal(m) struct monster *m;
{
...
void m_scaredy_move(m) struct monster *m;
{
  int dx = -sign(Player.x - m->x);
  int dy = -sign(Player.y - m->y);
  erase_monster(m);
...
void m_spirit_move(m) struct monster *m;
{
  int dx = sign(Player.x - m->x);
  int dy = sign(Player.y - m->y);
  erase_monster(m);
...
void m_flutter_move(m) struct monster *m;
{
...
void m_follow_move(m) struct monster *m;
{
  if (!m_statusp(m, HOSTILE))
    m_normal_move(m);
  else
...
void m_confused_move(m) struct monster *m;
{
...
void m_random_move(m) struct monster *m;
{
  int i, nx, ny, done = FALSE;
  erase_monster(m);
  for (i = 0; ((i < 8) && (!done)); i++)
...
void m_vanish(m) struct monster *m;
{
  if (m->uniqueness == COMMON)
  {
    strcpy(Str2, "The ");
...
void m_teleport(m) struct monster *m;
{
  erase_monster(m);
...
void m_move_leash(m) struct monster *m;
{
  m_simple_move(m);
  if (m->aux1 == 0)
  {
