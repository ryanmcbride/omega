...
void m_pulse(m) struct monster *m;
{
...
void movemonster(m, newx, newy) struct monster *m;
int newx, newy;
{
...
void m_pickup(m, o) struct monster *m;
struct object *o;
{
...
void m_dropstuff(m) struct monster *m;
{
...
void m_damage(m, dmg, dtype) struct monster *m;
int dmg, dtype;
{
  m_status_set(m, AWAKE);
...
void m_death(m) struct monster *m;
{
  pob corpse;
  pml ml;
  int x, y, found = FALSE;
...
void monster_move(m) struct monster *m;
{
  monster_action(m, m->movef);
}

void monster_strike(m) struct monster *m;
{
...
void monster_special(m) struct monster *m;
{
...
void monster_talk(m) struct monster *m;
{
  monster_action(m, m->talkf);
}

void monster_action(m, action) struct monster *m;
int action;
{
  int meleef;
...
void make_log_npc(npc) struct monster *npc;
{
  int i, n;
...
void m_trap_dart(m) struct monster *m;
{
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
    if (m->uniqueness != COMMON)
...
void m_trap_pit(m) struct monster *m;
{
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
    if (m->uniqueness != COMMON)
...
void m_trap_door(m) struct monster *m;
{
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
    if (m->uniqueness != COMMON)
...
void m_trap_abyss(m) struct monster *m;
{
  char Str1[80];
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_trap_snare(m) struct monster *m;
{
  char Str1[80];
  Level->site[m->x][m->y].locchar = TRAP;
  lset(m->x, m->y, CHANGED);
...
void m_trap_blade(m) struct monster *m;
{
  char Str1[80];
  Level->site[m->x][m->y].locchar = TRAP;
  lset(m->x, m->y, CHANGED);
...
void m_trap_fire(m) struct monster *m;
{
  char Str1[80];
  Level->site[m->x][m->y].locchar = TRAP;
  lset(m->x, m->y, CHANGED);
...
void m_fire(m) struct monster *m;
{
  char Str1[80];
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_trap_teleport(m) struct monster *m;
{
  char Str1[80];
  Level->site[m->x][m->y].locchar = TRAP;
  lset(m->x, m->y, CHANGED);
...
void m_trap_disintegrate(m) struct monster *m;
{
  char Str1[80];
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_trap_sleepgas(m) struct monster *m;
{
  char Str1[80];
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_trap_acid(m) struct monster *m;
{
  char Str1[80];
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_trap_manadrain(m) struct monster *m;
{
  char Str1[80];
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_water(m) struct monster *m;
{
  char Str1[80];
  if ((!m_statusp(m, INTANGIBLE)) &&
...
void m_abyss(m) struct monster *m;
{
  char Str1[80];
  if (los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_lava(m) struct monster *m;
{
  char Str1[80];
...
void m_altar(m) struct monster *m;
{
...
void strengthen_death(m) struct monster *m;
{
...
void m_no_op(m) struct monster *m;
{
}
