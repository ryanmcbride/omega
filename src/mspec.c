...
void m_sp_mp(m) struct monster *m;
{
...
void m_sp_ng(m) struct monster *m;
{
...
void m_sp_poison_cloud(m) struct monster *m;
{
...
void m_sp_explode(m) struct monster *m;
{
...
void m_sp_demon(m) struct monster *m;
{
  int mid;
...
void m_sp_acid_cloud(m) struct monster *m;
{
...
void m_sp_escape(m) struct monster *m;
{
  if (m_statusp(m, HOSTILE))
    m_vanish(m);
}

void m_sp_ghost(m) struct monster *m;
{
  if (m_statusp(m, HOSTILE))
  {
...
void m_sp_spell(m) struct monster *m;
{
  char action[80];
...
/* monsters with this have some way to hide, camouflage, etc until they 
   attack */
void m_sp_surprise(m) struct monster *m;
{
  if (m->attacked)
  {
    if (m_statusp(m, HOSTILE) &&
...
void m_sp_whistleblower(m) struct monster *m;
{
  if (m_statusp(m, HOSTILE))
  {
    alert_guards();
...
void m_sp_seductor(m) struct monster *m;
{
  if (m_statusp(m, HOSTILE))
  {
    if (m->uniqueness == COMMON)
...
void m_sp_demonlover(m) struct monster *m;
{
  if (distance(Player.x, Player.y, m->x, m->y) < 2)
    m_talk_demonlover(m);
}

void m_sp_eater(m) struct monster *m;
{
  int i;
...
void m_sp_dragonlord(m) struct monster *m;
{
  if (m_statusp(m, HOSTILE))
  {
    if (distance(m->x, m->y, Player.x, Player.y) < 2)
...
void m_sp_blackout(m) struct monster *m;
{
...
void m_sp_bogthing(m) struct monster *m;
{
  if (Player.status[IMMOBILE] &&
...
void m_sp_were(m) struct monster *m;
{
  int mid;
...
void m_sp_servant(m) struct monster *m;
{
...
void m_sp_av(m) struct monster *m;
{
  if (Player.mana > 0)
  {
...
void m_sp_lw(m) struct monster *m;
{
  if (random_range(2))
  {
...
void m_sp_angel(m) struct monster *m;
{
...
void m_sp_swarm(m) struct monster *m;
{
  if (random_range(5) == 1)
  {
    if (view_los_p(m->x, m->y, Player.x, Player.y))
...
void m_sp_raise(m) struct monster *m;
{
  int x, y;
  pol t;
...
void m_sp_mb(m) struct monster *m;
{
...
void m_sp_mirror(m) struct monster *m;
{
  int i, x, y;
  if (view_los_p(m->x, m->y, Player.x, Player.y))
  {
...
void m_illusion(m) struct monster *m;
{
...
void m_huge_sounds(m) struct monster *m;
{
...
void m_thief_f(m) struct monster *m;
{
  int i = random_item();
  if (random_range(3) == 1)
  {
...
void m_summon(m) struct monster *m;
{
  if ((distance(Player.x, Player.y, m->x, m->y) < 2) &&
      (random_range(3) == 1))
  {
...
void m_aggravate(m) struct monster *m;
{
...
void m_sp_merchant(m) struct monster *m;
{
  pml ml;
  if (m_statusp(m, HOSTILE))
...
void m_sp_court(m) struct monster *m;
{
  pml ml;
  if (m_statusp(m, HOSTILE))
  {
...
void m_sp_lair(m) struct monster *m;
{
  pml ml;
  if (m_statusp(m, HOSTILE))
  {
...
void m_sp_prime(m) struct monster *m;
{
  if (m_statusp(m, HOSTILE))
  {
