...
void plotspot(x, y, showmonster) int x, y, showmonster;
{
...
void plotmon(m) struct monster *m;
{
  if (!offscreen(m->y))
  {
...
void drawmonsters(display) int display;
{
  pml ml;
...
void erase_monster(m) struct monster *m;
{
...
Symbol getspot(x, y, showmonster) int x, y, showmonster;
{
...
void maddch(c) char c;
{
  waddch(Msgw, c);
  wrefresh(Msgw);
}

void display_death(source) char *source;
{
  clear();
  touchwin(stdscr);
  printw("\n\n\n\n");
...
void spreadroomlight(x, y, roomno) int x, y, roomno;
{
...
void spreadroomdark(x, y, roomno) int x, y, roomno;
{
  if (inbounds(x, y))
...
void display_inventory_slot(slotnum, topline) int slotnum;
int topline;
{
  WINDOW *W;
  char usechar = ')', idchar = '-';
...
int move_slot(oldslot, newslot, maxslot) int oldslot, newslot, maxslot;
{
...
void display_option_slot(slot) int slot;
{
  hide_line(slot);
