...
int screenmod(y) int y;
{
  return (y - ScreenOffset);
}

int offscreen(y) int y;
{
  return ((y < 0) ||
          (y < ScreenOffset) ||
...
int distance(x1, y1, x2, y2) int x1, y1, x2, y2;
{
  return (max(abs(x2 - x1), abs(y2 - y1)));
}

/* can you shoot, or move monsters through a spot? */
int unblocked(x, y) int x, y;
{
  if ((!inbounds(x, y)) ||
...
int m_unblocked(m, x, y) struct monster *m;
int x, y;
{
...
int view_unblocked(x, y) int x, y;
{
  if (!inbounds(x, y))
    return (FALSE);
...
int los_p(x1, y1, x2, y2) int x1, y1, x2, y2;
{
  int dx, dy;
  int major, minor;
  int error, delta, step;
...
int view_los_p(x1, y1, x2, y2) int x1, y1, x2, y2;
{
  int dx, dy;
  int major, minor;
  int error, delta, step;
...
char inversedir(dirindex) int dirindex;
{
  switch (dirindex)
  {
  case 0:
...
char *ordinal(number) int number;
{
...
int strprefix(prefix, s) char *prefix, *s;
{
...
char cryptkey(fname) char *fname;
{
  int pos, key = 0;
