...
void load_village(villagenum, populate) int villagenum;
int populate;
{
  int i, j;
  char site;
...
void make_food_bin(i, j) int i, j;
{
  pol tol;
  int k;
...
void make_merchant(i, j) int i, j;
{
  pml tml = ((pml)(checkmalloc(sizeof(mltype))));
...
void assign_village_function(x, y, setup) int x, y, setup;
{
  static int next = 0;
...
void special_village_site(i, j, villagenum) int i, j, villagenum;
{
  if (villagenum == 1)
  {
